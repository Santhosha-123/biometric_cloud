...
def UserLoginAction(request):
    ...
    if request.method == 'POST':
        ...
        with con:
            ...
            rows = cur.fetchall()
            for row in rows:
                print(str(row[2].decode()))
                if row[0] == username and password == row[1]:
                    finger = str(row[2].decode())
                    index = 1
                    break		
        if index == 1:
            output = "<tr><td>"
            output += '<input type="text" id="txtIsoTemplate" name="t1" size="100" class="form-control" value="'+finger+'" readonly> </input>'
            output += '</td></tr>'
            context= {'data':output}
...
def SignupAction(request):
    if request.method == 'POST':
        ...
        finger = request.POST.get('t7', False)
        output = "none"
        con = pymysql.connect(host='127.0.0.1',port = 3306,user = 'root', password = 'root', database = 'securebiometric',charset='utf8')
        with con:
            cur = con.cursor()
